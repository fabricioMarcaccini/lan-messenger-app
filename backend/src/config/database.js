...
export const db = {
    ...
    // Alias: keep backward compatibility with db.write()
...
export const cache = {
    ...
    async setPresence(userId, status) {
        if (fallbackEnabled) { memoryCache.set(`presence:${userId}`, status); return; }
        try {
    ...
    async clearPresence(userId) {
        if (fallbackEnabled) { memoryCache.delete(`presence:${userId}`); return; }
        try { await redis.del(`presence:${userId}`); } catch (e) { }
    },

    async getPresence(userId) {
        if (fallbackEnabled) return memoryCache.get(`presence:${userId}`) || 'offline';
        try {
