...
// Helper: Create users in bulk with seat lock validation
export const createBulkUsersWithSeatLock = async (companyId, usersArray) => {
    const BATCH_SIZE = 100;

    // Process in batches if array is too large, avoiding Postgres limits
    const batches = [];
    ...
    for (const batch of batches) {
        await db.transaction(async (client) => {
            ...
            const activeUsers = parseInt(usersResult.rows[0].count, 10);
            finalActiveUsers = activeUsers;

            // 3. Block if we don't have enough seats for this batch
            if (activeUsers + batch.length > maxSeats) {
                const available = maxSeats - activeUsers;
                throw Object.assign(
                    new Error(`Apenas ${available} assentos disponíveis. O arquivo contém mais usuários do que a cota do plano ($${maxSeats}). Faça upgrade do plano.`),
                    { status: 403, code: 'SEAT_LIMIT_REACHED', maxSeats, activeUsers }
                );
            }

            // 4. Bulk INSERT within the transaction
            for (const userData of batch) {
                try {
                    ...
                    if (err.code === '23505') { // unique violation
                        failedUsers.push({ email: userData.email, reason: 'E-mail ou username já existe' });
                        // No transaction rollback strictly for single-user failure inside the batch, 
                        // unless we want it fully atomic. We chose partial success here.
                    } else {
                        failedUsers.push({ email: userData.email, reason: err.message });
                    }
                }
            }
        });
    ...
    return {
        totalCreated,
        failedUsers,
        seatInfo: { maxSeats: finalMaxSeats, activeUsers: finalActiveUsers + totalCreated, seatsRemaining: finalMaxSeats - (finalActiveUsers + totalCreated) }
    };
};
